...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const logoUrl = siteBranding?.logoUrl?.trim();
        ...
        <div className="flex items-center justify-between h-18 sm:h-20 gap-4 py-2">
          ...
          <div className="hidden lg:flex items-center gap-2 font-cairo">
            ...
            {onOpenAddArticle && (
              <button
                type="button"
                id="nav-contribute-article-btn"
                onClick={onOpenAddArticle}
                className="px-3 py-1.5 rounded-xl border border-[#4A5D4E]/30 bg-[#4A5D4E]/10 hover:bg-[#4A5D4E]/20 text-[#4A5D4E] text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs active:scale-98"
                title="أضف مقالاً أو دراسة بحثية"
              >
                <FileText className="w-3.5 h-3.5 text-[#4A5D4E]" />
                <span>أضف مقالاً</span>
              </button>
            )}
          ...
          <div className="lg:hidden py-4 border-t border-[#E5E2D9] space-y-3 animate-in fade-in font-cairo">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 text-xs">
              ...
              {onOpenAddArticle && (
                <button
                  type="button"
                  id="mobile-nav-add-article-btn"
                  onClick={() => {
                    onOpenAddArticle();
                    setMobileMenuOpen(false);
                  }}
                  className="col-span-2 p-2.5 rounded-lg bg-[#4A5D4E]/10 text-[#4A5D4E] border border-[#4A5D4E]/30 text-right font-bold flex items-center justify-between"
                >
                  <div className="flex items-center gap-2">
                    <FileText className="w-4 h-4 text-[#4A5D4E]" />
                    <span>أضف مقالاً أو دراسة (مساهمة من القراء)</span>
                  </div>
                  <span className="text-[11px] bg-[#4A5D4E] text-white px-2 py-0.5 rounded-md">جديد</span>
                </button>
              )}
